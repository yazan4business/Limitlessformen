...
const ALL_COLORS = ["Black", "White", "Charcoal", "Stone", "Olive", "Indigo", "Grey"];
const CATEGORIES = ["Outerwear", "T-Shirts", "Knitwear", "Denim", "Bottoms", "Accessories"];
const STYLES = ["Casual", "Smart Casual", "Streetwear", "Formal", "Athletic"];

const COLOR_HEX = {
  Black: "#0a0a0a", White: "#f5f5f5", Charcoal: "#3a3a3a",
  Stone: "#a8a29e", Olive: "#5b5f43", Indigo: "#2c3550", Grey: "#87867f"
};
...
function Nav({ cartCount, cartBounce, wishlistCount, wishlistBounce, onNavigate, menuOpen, setMenuOpen, setCartOpen, scrolled, page }) {
  const links = [
...
function MobileBottomNav({ page, onNavigate, cartCount, cartBounce, wishlistCount, wishlistBounce, setCartOpen }) {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-30 glass-strong border-t border-neutral-800">
      <div className="flex items-stretch max-w-6xl mx-auto">
        ...
        <Item icon={<Search size={20} />} label="Shop" active={page === "shop"} onClick={() => onNavigate("shop")} />
        <Item
          icon={
        ...
        />
        <Item icon={<MessageCircle size={20} />} label="Contact" active={false} onClick={() => window.open(whatsappLink(), "_blank")} />
      </div>
    </div>
  );
}
...
function HomePage({ products, onNavigate, onAddToCart, wishlist, onToggleWishlist }) {
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 sm:gap-4">
          {STYLES.map((s, i) => (
            <Reveal key={s} delay={i * 70}>
              ...
              >
                <img src={img("style-" + s)} alt={s} className="w-full h-full object-cover" style={{ filter: "grayscale(1) contrast(1.1)" }} />
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
          {CATEGORIES.map((cat, i) => (
            <Reveal key={cat} delay={i * 60}>
              ...
              >
                <img src={img("cat-" + cat)} alt={cat} className="w-full h-full object-cover" style={{ filter: "grayscale(1) contrast(1.1)" }} />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-6">
          {featured.map((p, i) => (
            <Reveal key={p.id} delay={i * 80}>
              <ProductCard product={p} onNavigate={onNavigate} onAddToCart={onAddToCart} wishlist={wishlist} onToggleWishlist={onToggleWishlist} />
            </Reveal>
          ))}
        </div>
      </section>
...
function ProductCard({ product, onNavigate, onAddToCart, wishlist = [], onToggleWishlist }) {
        ...
        <div className="flex items-center justify-between mt-1">
          ...
          <div className="flex gap-1">
            {product.colors.slice(0, 4).map((c) => (
              <span key={c} title={c} className="w-3 h-3 rounded-full border border-neutral-700" style={{ background: COLOR_HEX[c] || "#888" }} />
            ))}
          </div>
        </div>
      </button>
...
function ShopPage({ products, initialCategory, initialStyle, onNavigate, onAddToCart, wishlist, onToggleWishlist }) {
        ...
        <div className="flex flex-wrap gap-2">
          {ALL_COLORS.map((c) => (
            <button
              ...
              className={`flex items-center gap-2 px-2.5 py-1.5 text-xs border transition-colors ${colorFilter.includes(c) ? "border-white bg-neutral-900" : "border-neutral-700 hover:border-neutral-400"}`}
            >
              <span className="w-3 h-3 rounded-full border border-neutral-600" style={{ background: COLOR_HEX[c] }} />
              {c}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1">
          ...
          {filtered.length === 0 ? (
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
              {filtered.map((p) => (
                <ProductCard key={p.id} product={p} onNavigate={onNavigate} onAddToCart={onAddToCart} wishlist={wishlist} onToggleWishlist={onToggleWishlist} />
              ))}
            </div>
          )}
        </div>
...
function ProductDetailPage({ product, products, onNavigate, onAddToCart, wishlist = [], onToggleWishlist }) {
            ...
            <div className="flex gap-2">
              {product.colors.map((c) => (
                <button
                  ...
                  style={{ background: COLOR_HEX[c], borderColor: color === c ? "#fff" : "transparent", transform: color === c ? "scale(1.1)" : "scale(1)" }}
                  title={c}
                />
              ))}
            </div>
...
function WishlistPage({ products, wishlist, onToggleWishlist, onNavigate, onAddToCart }) {
  ...
  return (
    <div className="max-w-sm mx-auto px-6 py-24">
      ...
      <p className="text-xs text-neutral-600 mt-6 text-center">
        {adminPassword === DEFAULT_ADMIN_PASSWORD ? `Default password: ${DEFAULT_ADMIN_PASSWORD}` : "Forgot your password? Ask whoever last changed it."}
      </p>
    </div>
  );
}
...
function AdminDashboard({ products, orders, onSaveProducts, onSaveOrders, adminPassword, onChangePassword, onLogout }) {
...
function ProductFormModal({ product, onClose, onSave }) {
  const [form, setForm] = useState(() => {
    const base = product
      ...
      : { name: "", category: CATEGORIES[0], price: "", compareAtPrice: "", styles: [], fabric: "", description: "", sizes: [], colors: [], variants: [], images: [], isNew: false, isFeatured: false, isBestSeller: false, isSoldOut: false };
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {ALL_COLORS.map((c) => (
                <button key={c} type="button" onClick={() => toggleArr("colors", c)} className={`flex items-center gap-1.5 px-2.5 py-1.5 text-xs border ${form.colors.includes(c) ? "border-white bg-neutral-900" : "border-neutral-700 text-neutral-400"}`}>
                  <span className="w-3 h-3 rounded-full border border-neutral-600" style={{ background: COLOR_HEX[c] }} />{c}
                </button>
              ))}
            </div>
          </div>
                ...
                {form.variants.map((v) => (
                  <div key={`${v.color}-${v.size}`} className="flex items-center justify-between gap-3 text-xs">
                    <span className="flex items-center gap-1.5 text-neutral-300">
                      <span className="w-2.5 h-2.5 rounded-full border border-neutral-600 flex-shrink-0" style={{ background: COLOR_HEX[v.color] }} />
                      {v.color} / {v.size}
                    </span>
                    <input
                      type="number"
